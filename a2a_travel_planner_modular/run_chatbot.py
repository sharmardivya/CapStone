...
already running them), then serves the chatbot at http://127.0.0.1:7860 — or, on a host such as
Render, on 0.0.0.0 and the port given in the PORT environment variable.
...
def main() -> None:
    ...
    launch_chatbot(open_browser=not args.no_browser and not config.IS_HOSTED)
