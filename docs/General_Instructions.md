...
- **Language:** Python
- **Version Requirements:** Python 3.12 or later (`requires-python = ">=3.12"`). Developed and tested on 3.12.12, which `.python-version` requests.
...
- **Package Manager:** `uv` (0.12.9) for development — `pyproject.toml` declares dependencies and `uv.lock` pins exact versions. `requirements.txt` is exported from `uv.lock` so that `pip` (used by Render) installs the same versions.
...
├── pyproject.toml / uv.lock        # Dependencies (development)
├── requirements.txt                # Dependencies exported for pip / Render
...
- `CHATBOT_PORT`: port for the browser interface when running locally; defaults to `7860`.
- `PORT`: set automatically by Render. When present, the chatbot listens on `0.0.0.0` and this port, and no browser is opened.
- `PYTHON_VERSION`: set on Render to `3.12.12` so the build uses the tested Python version.
...
Without `uv`, or on Render:

```bash
pip install -r requirements.txt
```

Deployment to Render is described step by step in the project `README.md`.
...
- Bind the four agents to `127.0.0.1` at all times, so they are never reachable from the network. The chatbot binds to `127.0.0.1` locally, and to `0.0.0.0` only when a hosting platform sets `PORT`.
